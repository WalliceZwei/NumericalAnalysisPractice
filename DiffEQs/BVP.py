...
def ShootingMethod(fun, starttime, ya, endtime, yb, h):
    ...
    def partial(slopeinput):
        return (
            ExplicitTrapezoid(
                fun,
                (starttime, np.array([ya, slopeinput])),
                h,
                math.ceil(timeelapsed / h) + 1,
            )[-1][1][0]
            - yb
        )

    conditions = NewtonsMethod(partial, 1, 0.00001)
    print(conditions)
    return ExplicitTrapezoid(
        fun, (starttime, np.array([ya, conditions])), h, math.ceil(timeelapsed / h) + 1
    )


solution_points = ShootingMethod(func, 0, 1, 1, 3, 0.05)
